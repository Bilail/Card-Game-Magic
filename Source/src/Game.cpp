...
// Phase de jeux des cartes d'enchantements :
if (cardToPlay == "Blanc"){
    // On gagne un point de vie à chaque fois que on joue une carte blanche
}
if (cardToPlay == "Bleu"){
    // Prendre le controle de la créature ciblé
}
if (cardToPlay == "Noir"){
// Quand une unité meurt le jours perd 1 hp
}
if (cardToPlay == "Rouge"){
// +1 d'attaque à chaque phase d'attaque
}
if (cardToPlay == "Vert"){
// le terrain enchanté rapporte double de ressource, et si créature alors +X/+X par terrain forest
    std::cout
      << "Indiquer que vous souahitez enchanté  (tapez \"aucune\" pour ne rien jouer) : ";
            bool validInput = false;
            while (!validInput) {
                std::string cardToPlay = "";
                std::getline(std::cin, cardToPlay);
                if (lower(cardToPlay) != "aucune") {
                    ...
                    if (validInput) {
                        std::cout << "\nVous venez d'enchanter la carte " << StrColor::print(cardToPlay, cardColor) << std::endl;
                    } else {
                        std::cout << "Le nom de la carte est inconnu, veuillez réessayer : ";
                    }
                }
                else {
                    std::cout << "Vous avez décidé de ne pas enchanter de carte.\n";
                }
            }
}
